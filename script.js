...
function loadData() {
    ...
    const savedLevel = localStorage.getItem('wordMasterLevel');
    if (savedLevel) {
...
function saveCurrentLevel() {
    localStorage.setItem('wordMasterLevel', currentLevel);
}

// FIXED: Go to selected level - Now works correctly!
function goToLevel() {
    const selectedLevel = parseInt(levelSelector.value);
    
    if (selectedLevel >= 1 && selectedLevel <= 30) {
        ...
        // Reset score and streak for new level
        score = 0;
        streak = 0;
        scoreDisplay.textContent = score;
        streakDisplay.textContent = streak;
        
        // CRITICAL FIX: Force reload word from the NEW level
        loadWordFromCurrentLevel();
        
        showMessage(`✨ Jumped to LEVEL ${currentLevel}! ✨`, 'success');
        showPopup(`🚀 LEVEL ${currentLevel}`, 'correct');
    }
}

// FIXED: Load word from the current level - This is the key fix!
function loadWordFromCurrentLevel() {
    const levelWords = wordDatabase[currentLevel];
    
    // Check if level exists
    if (!levelWords || levelWords.length === 0) {
        scrambledWordDiv.textContent = "COMPLETE!";
        hintSpan.innerHTML = "Congratulations! You've mastered all words!";
        gameActive = false;
        return;
    }
    
    // Get random word from current level
    ...
    hintSpan.innerHTML = currentHint;
    guessInput.value = "";
...
function resetTimer() {
    ...
    timerFill.style.background = "linear-gradient(90deg, #4ecdc4, #ffe66d)";
...
function correctAnswer() {
    ...
    // Level up if score threshold reached and not at max level
    if (score >= currentLevel * 500 && currentLevel < 30) {
        currentLevel++;
        levelDisplay.textContent = currentLevel;
        levelNumber.textContent = currentLevel;
        levelSelector.value = currentLevel;
...
function useHint() {
    ...
    hintSpan.innerHTML = `${currentHint}<br><br><strong>🔮 Revealed letters:</strong> ${revealedWord}`;
...
function init() {
    createBubbles();
    loadData();
    loadWordFromCurrentLevel();  // FIXED: Using the correct function
}
